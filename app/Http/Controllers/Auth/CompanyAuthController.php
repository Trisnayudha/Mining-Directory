...
class CompanyAuthController extends Controller
{
    ...
    public function loginPassword(Request $request)
    {
        try {
            ...
            // Attempt to authenticate the user with the company guard
            if (!$token = Auth::guard('company')->attempt($request->only('email', 'password'))) {
                // Return error response if authentication fails
                return $this->sendResponse('Unauthorized', null, 401);
            }

            // Get the authenticated user from the Auth facade
    ...
    public function requestOtp(Request $request)
    {
        ...
        try {
            ...
            if ($type == 'email') {
                ...
                $sendEmail->subject = "Company: OTP Login";
    ...
    public function verifyOtp(Request $request)
    {
        ...
        try {
            ...
            if ($cachedOtp && $cachedOtp == $userInputOtp) {
                ...
                $user = Company::where('email', $identifier)->first();
                if (!$user) {
                    return $this->sendResponse('User not found', null, 404);
                }
